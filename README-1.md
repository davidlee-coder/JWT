...
**Level** — Expert  
**Category** — JWT Attacks / Algorithm Confusion  
**PortSwigger Link** — https://portswigger.net/web-security/jwt/lab-jwt-algorithm-confusion-no-exposed-key  
**Completed** — February 10 2026  
**Tools** — Burp Repeater, JWT Editor (for signing), Burp Decoder
...
During this initial probe, I identified that the application relies on JSON Web Tokens (JWT) for session management. Using the Burp JWT Editor extension, I decoded the session cookie and performed a deep-dive into the token's header and payload.

I found that the server was utilizing the RS256 (RSA Signature with SHA-256) asymmetric algorithm. This immediately raised a red flag: if I could trigger a 'Type Confusion' by switching the algorithm to HS256, I might be able to trick the server into using its own public key as a symmetric HMAC secret.
